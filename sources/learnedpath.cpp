...
void LearnedPathHandler::getLearnedCommands(const int timestep, double & left_motor_command_o, double & right_motor_command_o,
    ...
    double path_unlikeness      = abs(last_known_angle_to_sound - learned_paths[0].angle_to_sound[timestep])/ 360   * 100
                                + abs(angle_to_obst_            - learned_paths[0].angle_to_obst[timestep]) / 360   * 10
                                + abs(dist_to_obst_             - learned_paths[0].dist_to_obst[timestep])  / 10000 * 10;
    int closest_path_index = 0;
    double path_unlikeness_temp;
    for(size_t i = 1; i < learned_paths.size(); i++){
        path_unlikeness_temp    = abs(last_known_angle_to_sound - learned_paths[i].angle_to_sound[timestep])/ 360   * 100
                                + abs(angle_to_obst_            - learned_paths[i].angle_to_obst[timestep]) / 360   * 10
                                + abs(dist_to_obst_             - learned_paths[i].dist_to_obst[timestep])  / 10000 * 10;
        if(path_unlikeness_temp < path_unlikeness){
            closest_path_index = i;
            path_unlikeness = path_unlikeness_temp;
        }
    }
    left_motor_command_o = learned_paths[closest_path_index].left_motor_command[timestep];
    right_motor_command_o = learned_paths[closest_path_index].right_motor_command[timestep];
}
